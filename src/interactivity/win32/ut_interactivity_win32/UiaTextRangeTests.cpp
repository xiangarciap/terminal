...
class UiaTextRangeTests
{
    ...
    TEST_METHOD(GeneratedMovementTests)
    {
        ...
        // Read tools\TestTableWriter\README.txt for more details
        ...
        VERIFY_ARE_EQUAL(_pTextBuffer->GetSize(), bufferSize);
        ...
        const til::point midHistoryLeft{ bufferSize.Left(), midHistory.y() };

        COORD midHistoryM1C = midHistory;
        for (int i = 0; i < 1; ++i)
        {
            bufferSize.DecrementInBounds(midHistoryM1C);
        }

        const til::point midHistoryM1L{ bufferSize.Left(), midHistory.y() - 1 };

        COORD midHistoryM4C = midHistory;
        for (int i = 0; i < 4; ++i)
        {
            bufferSize.DecrementInBounds(midHistoryM4C);
        }

        const til::point midHistoryM4L{ bufferSize.Left(), midHistory.y() - 4 };

        COORD midHistoryM5C = midHistory;
        for (int i = 0; i < 5; ++i)
        {
            bufferSize.DecrementInBounds(midHistoryM5C);
        }

        const til::point midHistoryM5L{ bufferSize.Left(), midHistory.y() - 5 };

        COORD midHistoryP1C = midHistory;
        for (int i = 0; i < 1; ++i)
        {
            bufferSize.IncrementInBounds(midHistoryP1C);
        }

        const til::point midHistoryP1L{ bufferSize.Left(), midHistory.y() + 1 };

        COORD midHistoryP2C = midHistory;
        for (int i = 0; i < 2; ++i)
        {
            bufferSize.IncrementInBounds(midHistoryP2C);
        }

        const til::point midHistoryP2L{ bufferSize.Left(), midHistory.y() + 2 };

        COORD midHistoryP5C = midHistory;
        for (int i = 0; i < 5; ++i)
        {
            bufferSize.IncrementInBounds(midHistoryP5C);
        }

        const til::point midHistoryP5L{ bufferSize.Left(), midHistory.y() + 5 };

        COORD midHistoryP6C = midHistory;
        for (int i = 0; i < 6; ++i)
        {
            bufferSize.IncrementInBounds(midHistoryP6C);
        }

        const til::point midHistoryP6L{ bufferSize.Left(), midHistory.y() + 6 };

        COORD midTopM1C = midTop;
        for (int i = 0; i < 1; ++i)
        {
            bufferSize.DecrementInBounds(midTopM1C);
        }

        COORD midTopM4C = midTop;
        for (int i = 0; i < 4; ++i)
        {
            bufferSize.DecrementInBounds(midTopM4C);
        }

        COORD midTopM5C = midTop;
        for (int i = 0; i < 5; ++i)
        {
            bufferSize.DecrementInBounds(midTopM5C);
        }

        COORD midTopP1C = midTop;
        for (int i = 0; i < 1; ++i)
        {
            bufferSize.IncrementInBounds(midTopP1C);
        }

        const til::point midTopP1L{ bufferSize.Left(), midTop.y() + 1 };

        COORD midTopP2C = midTop;
        for (int i = 0; i < 2; ++i)
        {
            bufferSize.IncrementInBounds(midTopP2C);
        }

        COORD midTopP5C = midTop;
        for (int i = 0; i < 5; ++i)
        {
            bufferSize.IncrementInBounds(midTopP5C);
        }

        const til::point midTopP5L{ bufferSize.Left(), midTop.y() + 5 };

        COORD midTopP6C = midTop;
        for (int i = 0; i < 6; ++i)
        {
            bufferSize.IncrementInBounds(midTopP6C);
        }

        COORD originP1C = origin;
        for (int i = 0; i < 1; ++i)
        {
            bufferSize.IncrementInBounds(originP1C);
        }

        const til::point originP1L{ bufferSize.Left(), origin.y() + 1 };

        COORD originP2C = origin;
        for (int i = 0; i < 2; ++i)
        {
            bufferSize.IncrementInBounds(originP2C);
        }

        const til::point originP2L{ bufferSize.Left(), origin.y() + 2 };

        COORD originP5C = origin;
        for (int i = 0; i < 5; ++i)
        {
            bufferSize.IncrementInBounds(originP5C);
        }

        const til::point originP5L{ bufferSize.Left(), origin.y() + 5 };

        COORD originP6C = origin;
        for (int i = 0; i < 6; ++i)
        {
            bufferSize.IncrementInBounds(originP6C);
        }

        const til::point originP6L{ bufferSize.Left(), origin.y() + 6 };
        ...
        const std::array<GeneratedMovementTest, 90> _movementTests{
